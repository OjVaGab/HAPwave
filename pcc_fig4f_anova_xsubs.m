...
for ss = 1:nr_subs  % loop over subjects
    ...
    all_out(ss).Vsnr        = NaN(size(all_out(ss).crp_out));
    ...
    for kk = 1:length(these_measured_sites)
        ...
        for ll = 1:length(these_stim_sites)
            if ~isempty(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).data)   % we have CRPs
                ...
                all_out(ss).tR(these_measured_sites(kk), these_stim_sites(ll))     = median(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).crp_parms.tR);    % CRP significant response duration
                all_out(ss).Vsnr(these_measured_sites(kk), these_stim_sites(ll))     = median(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).crp_parms.Vsnr);% CRP signal to noise ratio
                ...
                sig_timepoints = find(all_out(ss).tt>0.015 & all_out(ss).tt<=all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).crp_parms.tR);                   % Signiticant timepoints
...
t_win_norm      = [0.015 0.500];        % window for vector length normalization
            ...
            for kk = 1:length(these_measured_sites)
                ...
                for ll = 1:length(these_stim_sites)
                    if ~isempty(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).data) % ~ same stim/recording site
                        ...
                        out(measure_ind,stim_ind).Vsnr(resp_counter, :)     = all_out(ss).Vsnr(these_measured_sites(kk), these_stim_sites(ll));
...
%set CRP param to feed the model

pcc_hc_crp              = out(3,1).cod;            % measured PCC (3) and stim HC (1)
pcc_hc_stimgroup        = cell(size(pcc_hc_crp));  % hc stim gets code 1 
pcc_hc_measgroup        = cell(size(pcc_hc_crp));  % pcc measure gets code 3
[pcc_hc_stimgroup{:}]   = deal('HC');
[pcc_hc_measgroup{:}]   = deal('PCC');
sub_ind1                = out(3,1).subj_ind;        % HC-PCC connection 

acc_hc_crp              = out(4,1).cod;             % measured ACC (4) and stim HC (1)
acc_hc_stimgroup        = cell(size(acc_hc_crp));   % hc stim gets code 1
acc_hc_measgroup        = cell(size(acc_hc_crp));   % acc measure gets code 4
[acc_hc_stimgroup{:}]   = deal('HC');
[acc_hc_measgroup{:}]   = deal('ACC');
sub_ind2                = out(4,1).subj_ind;        % HC-ACC connection 

pcc_amg_crp             = out(3,2).cod;             % measured PCC (3) and stim Amygdala (2)
pcc_amg_stimgroup       = cell(size(pcc_amg_crp));  % amg stim gets code 2
pcc_amg_measgroup       = cell(size(pcc_amg_crp));  % pcc measure gets code 3
[pcc_amg_stimgroup{:}]  = deal('AMG');
[pcc_amg_measgroup{:}]  = deal('PCC');       
sub_ind3                = out(3,2).subj_ind;        % Amg-PCC connection

acc_amg_crp             = out(4,2).cod;             % measured ACC (4) and stim Amygdala (2)
acc_amg_stimgroup       = cell(size(acc_amg_crp));  % amg stim gets code 2
acc_amg_measgroup       = cell(size(acc_amg_crp));  % acc measure gets code 4
...
%% Different LME models for best fit to test for interation of COD
...
%% Compare LME models
% % Other LME models where tested for a better fit
...
lme_pcc     = fitlme(tbl,'ccep_val ~ 1 + stim_site + (stim_site|sub_ind)','FitMethod','REML');
...
set(gca,'YTick',[0:.2:1],'XTick',[1:4],'XTickLabel',{'H-ACC','A-ACC','H-PCC','A-PCC'})
